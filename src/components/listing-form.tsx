...
export function ListingForm({
  ...
  //
  // ⚠️ ПУСТОЕ значение границу не двигает, и это не придирка. На сайте здесь
  // выпадающие списки, пустыми они не бывают; тут поля ввода. Без проверки на
  // пустоту стирание максимума давало `Number('') === 0`, ноль оказывался ниже
  // минимума — и минимум стирался следом. Подача падала с «обязательное поле» на
  // минимуме, хотя человек правил максимум.
  //
  // Пределы при пустом соседе взяты у сайта: снизу 1, сверху 10.
  const changeMinGuests = (value: string) => {
    setMinGuests(value)
    if (value && Number(value) > Number(guests || 10)) setGuests(value)
  }
  const changeMaxGuests = (value: string) => {
    setGuests(value)
    if (value && Number(value) < Number(minGuests || 1)) setMinGuests(value)
  }
    ...
    >
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        {/* Карта идёт сразу под адресом, как на сайте: кнопка ищет точку по
            ...
            ⚠️ В запрос идут ТОЛЬКО адрес и город. `district` класть нельзя: с
            появлением выбора места он стал КЛЮЧОМ метки (`port_baku`, `28_may`),
            а не текстом. Геокодер таких слов не знает, не находит ничего и
            отдаёт центр Баку — карта показывает «поиск локации не удался».
            Сайт шлёт один адрес; город добавлен, чтобы сузить поиск. */}
        <LocationPicker
          value={coordinates}
